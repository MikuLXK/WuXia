...
- **ID规范**: 所有 \`ID/id\` 仅允许 \`NPC001\` / \`Item001\` 这类“简短前缀+三位数字”，禁止英文全称与拼音拼写 ID。
...
- **结构唯一性**：正文只能使用一个 \`logs\` 字段作为统一容器；禁止新增 \`narration_logs\` / \`dialogue_logs\` / \`speaker_logs\` 等并行字段。
...
- **旁白中严禁夹带角色台词**：\`sender=旁白\` 的 \`text\` 不得出现任何直接引语（如 \`“...”\` / \`「...」\` / \`『...』\`）与“角色名+冒号”发言格式（如 \`沈清婉：...\`）。
- 若旁白句子里出现“某人说道/提醒/低声道 + 引号内容”，必须拆分为：上一条旁白（动作/环境）+ 下一条角色对话（仅台词）。
- **角色发言必须另起一行**：每次发言必须独占一条 \`logs\` 数组元素（同一大 \`logs\` 字段内），禁止把多个角色台词和旁白压进同一条 \`text\`。
- 若检测到 \`sender=旁白\` 且 \`text\` 中出现引号台词，视为结构违规；必须在本轮输出前重写为“旁白条 + 角色条”。
- “拆成两条 logs”仅指：\`logs\` 数组中新增两个对象元素，不是新增第二个 \`logs\` 字段。
...
- \`text\` 字段格式要求：
  - 使用全角分隔符 \`｜\`，字段顺序固定，不得换序。
  - \`触发对象\` 为必填字段，必须显式区分玩家或 NPC（如：\`触发对象 玩家:玩家名称\`、\`触发对象 NPC:沈清婉\`）。
